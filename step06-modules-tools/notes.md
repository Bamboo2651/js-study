...
- [x] `export` / `import` を使ってファイルを分割できる
- [x] 名前付きエクスポートとデフォルトエクスポートの違いを説明できる
- [x] `npm init` でプロジェクトを初期化できる
- [x] Vite で開発サーバーを起動できる
- [x] `.env` でAPIキーを管理できる
- [x] ブレークポイントを使ってデバッグできる
- [x] Network パネルでAPIのレスポンスを確認できる
